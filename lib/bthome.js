...
function processDataType(type, data, processedData) {
  switch(type) {
    ...
    case 0x17: // carbon monoxide
      if(data.length < 1) return -1;
      processedData.isCarbonMonoxideDetected = [ (data.readUInt8() > 0) ];
      return 1;
    case 0x18: // cold
      if(data.length < 1) return -1;
      // TODO: add property?
    ...
    case 0x1e: // light
      if(data.length < 1) return -1;
      processedData.isLightDetected = [ (data.readUInt8() > 0) ];
      return 1;
    case 0x1f: // lock
      if(data.length < 1) return -1;
      // TODO: add property?
    ...
    case 0x29: // smoke
      if(data.length < 1) return -1;
      processedData.isSmokeDetected = [ (data.readUInt8() > 0) ];
      return 1;
    case 0x2a: // sound
      if(data.length < 1) return -1;
      // TODO: add property?
    ...
    case 0x42: // duration
      if(data.length < 3) return -1;
      combine(processedData, 'duration', 'durations',
              data.readUIntLE(0, 3) / 1000);
      return 3;
    case 0x43: // current
      if(data.length < 2) return -1;
