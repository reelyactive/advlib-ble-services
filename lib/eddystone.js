...
const EDDYSTONE_UID_TRUNCATED_FRAME_LENGTH = 18;
const EDDYSTONE_UID_FRAME_LENGTH = 20;
const EDDYSTONE_TLM_FRAME_LENGTH = 14;
const EDDYSTONE_ETLM_FRAME_LENGTH = 18;
const EDDYSTONE_EID_FRAME_LENGTH = 10;
...
const FIND_HUB_URI = "https://sniffypedia.org/Service/Find_Hub/";
...
function process(data) {
  ...
  switch(frameType) {
    ...
    case 0x30:
      return processEddystoneEID(buf);
    case 0x40: // Nearby mode?
    case 0x41: // Separated mode?
      return processFindHub(buf);
  }
...
function processEddystoneUID(data) {
  let isInvalidLength = ((data.length !== EDDYSTONE_UID_FRAME_LENGTH) &&
                        (data.length !== EDDYSTONE_UID_TRUNCATED_FRAME_LENGTH));
  if(isInvalidLength) {
    return null;
  }
  ...
  let deviceId = namespace + utils.SIGNATURE_SEPARATOR + instance;

  return { txPower: txPower, deviceIds: [ deviceId ] };
}
...
function processEddystoneURL(data) {
  ...
  return { txPower: txPower, uri: url };
}
...
function processEddystoneTLM(data) {
  let isInvalidLength = ((data.length !== EDDYSTONE_TLM_FRAME_LENGTH) &&
                         (data.length !== EDDYSTONE_ETLM_FRAME_LENGTH));
  if(isInvalidLength) {
    return null;
  }
  ...
  switch(version) {
    case 0x00: // Unencrypted
      ...
      let txCount = data.readUInt32BE(ADV_CNT_OFFSET);
    ...
    case 0x01: // Encrypted
      let encrypted = {
          data: data.toString('hex', ETLM_OFFSET, SALT_OFFSET),
          salt: data.readUInt16BE(SALT_OFFSET),
          checksum: data.readUInt16BE(MIC_OFFSET),
...
/**
 * Process Eddystone-EID service data.
 * @param {Object} data The raw service data as a Buffer.
 * @return {Object} The processed Eddystone-EID data as JSON.
 */
function processEddystoneEID(data) {
  let isInvalidLength = (data.length !== EDDYSTONE_EID_FRAME_LENGTH);
  if(isInvalidLength) {
    return null;
  }

  let txPower = data.readInt8(TX_POWER_OFFSET);
  let eid = data.toString('hex', EID_OFFSET);
  let deviceId = eid + utils.SIGNATURE_SEPARATOR + '0';// TODO: 64-bit EID type?
...
/**
 * Process Find Hub service data.
 * @param {Object} data The raw service data as a Buffer.
 * @return {Object} The processed Find Hub data as JSON.
 */
function processFindHub(data) {
  return { uri: FIND_HUB_URI };
}
